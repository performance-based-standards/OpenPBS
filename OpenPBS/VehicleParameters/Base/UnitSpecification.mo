...
record UnitSpecification "Specification data, typically from bilregistret"
  ...
  parameter Modelica.SIunits.Length coupling_distance_EU "Kopplingsavstånd EU";
  ...
  parameter Modelica.SIunits.Mass kerb_mass "Tjänstevikt inkl påbyggnad";
  ...
  parameter Modelica.SIunits.Length load_space_length "Length of load space";
  ...
  // Tire data
  parameter Real cornering_coefficient=5.5 "Cornering coefficient: C_alpha/Fz";

  // Axle data
  parameter Integer n_wheels_per_axle[n_axles] "Number of wheels per axle";
  parameter Modelica.SIunits.Length axle_width[n_axles]
    "Axle width per axle (outer wheel ends)";
  parameter Integer axle_groups[n_axles] "Axle group assignment for each axle";
