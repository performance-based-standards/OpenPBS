function [] = animateLSSP(rx,ry,delta,pz,w,A,B)
%ANIMATE e
...
hold on;
set(ax,'XLim',[25 85],'YLim',[-2.5 57.5]);
axis manual;
axis equal;

i=1;
[chassis,axles,wheels] = drawVehicle(squeeze(rx(i,:,:)),squeeze(ry(i,:,:)),squeeze(delta(i,:,:)),pz(i,:),w,A,B,ax);
set(ax,'XLim',[25 85],'YLim',[-2.5 57.5]);
...
hLstn = addlistener(b,'ContinuousValueChange',@(es,ed) updateVehicle(squeeze(rx(floor(es.Value),:,:)),squeeze(ry(floor(es.Value),:,:)),squeeze(delta(floor(es.Value),:,:)),pz(floor(es.Value),:),w,A,B,chassis,axles,wheels));
